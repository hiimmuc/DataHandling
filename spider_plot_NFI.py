...
def plot_spider_graph(mean, std, save_name, title):
...
for i in range(5):
    ...
    df_selected = df.iloc[:, start_idx:start_idx + 5]
    # if len(df_selected) == 0:
    #     print(df[col_name], '\n', uniq_val)
    big5_mean = list(df_selected.mean().to_dict().values())
    big5_std = list(df_selected.std().to_dict().values())

    plot_spider_graph(big5_mean, big5_std,
                      f"eHMI_{i}",
                      title=f"eHMI {i}")

    r.add_text(f">>: ")
    r.add_picture(
        f"./figures/survey_spider_images/eHMI_{i}.png")

document.save(f'./plot_documents/survey_spider_charts_BFI_{n_rows}.docx')
